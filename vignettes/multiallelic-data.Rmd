...
myproj <- malecot_file("tutorial3_myproj.rds")
```
...
Notice that the autocorrelation falls off much more slowly than it did under the bi-allelic MCMC, and hence the effective sample size is quite small. This is one case where we should re-run the MCMC with a larger number of `samples` to increase the chance that the MCMC has explored the space well. For the sake of this tutorial we will save time by loading in results obtained by re-running this MCMC with `samples = 1e5`:
...
```{r}
# load results
myproj <- malecot_file("tutorial3_myproj2.rds")
...
We now mave a much larger effective sample size, giving us greater confidence that the MCMC has explored the space well.
...
The same gains in precision apply to the estimated mean COI per subpopulation:

```{r, fig.height=4, fig.width=5}
plot_COI_mean(myproj, K = 5)
```

The model clearly detects that there are mean COI differences between demes.
...
In the [previous tutorial](https://bobverity.github.io/MALECOT/articles/complex-priors.html) we saw how different priors could be used to define skewed allele frequency distributions, and how different prior shapes could be explored using the `plot_prior_p()` function. The same is true for multi-allelic data, except now the prior plots look different because the prior is *J*-dimensional, where *J* is the number of alleles. In multi-allelic plots the *J* dimensions are visualised using a [parallel coordinates plot](https://en.wikipedia.org/wiki/Parallel_coordinates), in which axes are plotted in parallel rather than perpendicular. The prior is visualised using 1000 random draws, which correspond to 1000 simulated trajectories through this plot.

The following prior is skewed in favour of the second allele. The expected allele frequency for allele *j* is equal to `lambda[j]/sum(lambda)`, which in this case yields an expectation of 0.375 for the second allele:

```{r, fig.height=3, fig.width=6}
plot_prior_p(lambda = c(1,6,2,3,4))
```

The prior uncertainty is dictated by the magnitude of `lambda`, with smaller uncertainty at large values. For example, we could define a second prior with exactly the same expected allele frequencies as the first but lower uncertainty by multiplying all `lambda` values by 10:

```{r, fig.height=3, fig.width=6}
plot_prior_p(lambda = c(10,60,20,30,40))
```

When thinking about this prior it is useful to imagine that each subpopulation has its own allele frequencies drawn from one of these trajectories. Therefore if the prior is very precise (like the one above) then we are stating that we expect subpopulations to look very similar in their composition.
