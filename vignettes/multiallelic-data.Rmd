...
The prior uncertainty is dictated by the magnitude of `lambda`, with larger values leading to reduced uncertainty. For example, we could define a second prior with exactly the same expected allele frequencies but lower uncertainty by multiplying all `lambda` values by 10:

```{r, fig.height=3, fig.width=6}
plot_prior_p(lambda = c(10,60,20,30,40))
```

When thinking about the prior on allele frequencies it is useful to imagine that each subpopulation has its own allele frequencies given by one of these trajectories. Therefore if the prior is very precise (like the one above) then we are stating that we expect subpopulations to look very similar in their composition. If this seems unlikely *a priori* then we should use a smaller value of *lambda*.
