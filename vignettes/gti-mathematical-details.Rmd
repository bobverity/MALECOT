...
Most of this derivation is taken from [Supplementary Material 1](http://www.genetics.org/content/203/4/1827.supplemental) of (ref), which also contains derivations of common model comparison statistics.
...
Finally, this quantity must be related back to the model evidence. It is easy to demonstrate that the integral of (X) with respect to $\beta$ over the range $[0,1]$ is equal to the logarithm of the model evidence:
...
$$
\begin{eqnarray}
	P_{\beta,\alpha}(\theta \:|\: x) &= \dfrac{\mbox{Pr}( x \:|\: \theta)^{\beta^\alpha} \: \mbox{Pr}(\theta)}{u( x \:|\: \beta, \alpha)} \;.
\end{eqnarray}
$$
...
$$
\begin{eqnarray*}
	u( x \:|\: \beta, \alpha) &= \displaystyle\int_{\scriptsize\theta} \mbox{Pr}( x \:|\: \theta)^{\beta^\alpha} \: \mbox{Pr}(\theta) \:d\theta \;.
\end{eqnarray*}
$$
It is no more difficult to obtain draws from this distribution than it is to obtain draws from the original power posterior distribution. Continuing with the standard TI derivation we obtain:
...
\begin{eqnarray}
	...
	&=& \mbox{E}_{\scriptsize\theta \,|\, x, \beta, \alpha}\Big[ \alpha\beta^{\alpha-1} \log[ \mbox{Pr}( x \:|\: \theta ) ] \Big] \;.
\end{eqnarray}
$$

Notice that the log-likelihood in the expectation is now weighted by $\alpha\beta^{\alpha-1}$. It is still the case that the integral of $\frac{d}{d\beta} \log[u( x \:|\: \beta, \alpha )]$ over the interval $[0,1]$ brings us to the log-evidence, and so the rest of the TI method remains essentially unchanged. We can define the statistic

$$
\begin{eqnarray}
	\widehat{D}_{\beta,\alpha} = \frac{1}{t}\sum_{m=1}^t \alpha\beta^{\alpha-1} \log\left[ \mbox{Pr}( x \:|\: \theta_m^{\beta^\alpha} ) \right] \;.
\end{eqnarray}
$$
for a range of $\beta_i$, and we can carry out numerical integration over these values as before. The fact that the expectation in (X) is weighted by $\alpha\beta^{\alpha-1}$ means that values towards the prior end of the spectrum are down-weighted relative to values near the posterior end of the spectrum, and in particular the new path is constrained to equal zero at the point $\beta = 0$ whenever $\alpha > 1$. This results in a much smoother path, and reduces both statistical error and discretisation error.

In summary, what we refer to as *generalised* thermodynamic integration here is any method by which the power $\beta$ is replaced by $f(\beta)$ to achieve a different weighting over the elements of the integral path. [REF]. In MALECOT we use the function $f(\beta) = \beta^\alpha$ as above, and the argument `GTI_pow` in the `run_mcmc()` function is exactly equal to $\alpha$.



### References

Hug, Sabine, et al. "An adaptive scheduling scheme for calculating Bayes factors with thermodynamic integration using Simpson’s rule." Statistics and Computing 26.3 (2016): 663-677.
