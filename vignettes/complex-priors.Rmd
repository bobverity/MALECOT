...
1. `lambda` can be a single value. In this case the same shape parameter is assumed for all alleles, making the prior symmetric. The same prior is applied to all loci.
...
By default `lambda` is defined using the first method with a value of 1, which places equal weight on every allele frequency between 0 and 1 at every locus. Here we will instead use the second method to place a skewed prior on allele frequencies.
