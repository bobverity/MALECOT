...
myproj <- new_set(myproj, name = "uniform model", COI_model = "uniform", COI_max = 20,
                  estimate_error = FALSE, e1 = 0, e2 = 0)
```
...
Note that all plots produced by MALECOT are produced as [ggplot](https://ggplot2.tidyverse.org/) objects, meaning they can be stored and modified later - for example by adding titles, legends etc.
...
Alongside the posterior allocation plot, we can plot the posterior COI of all samples. The example below uses the `ggplot2` package to add the true (simulated) COIs to the plot in the form of red crosses.
...
myproj <- new_set(myproj, name = "nb model", COI_model = "nb", COI_max = 20,
                  ...
                  estimate_error = TRUE, e1_max = 0.1, e2_max = 0.1)
myproj
```
